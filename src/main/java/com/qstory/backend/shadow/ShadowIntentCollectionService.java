...
public class ShadowIntentCollectionService {
    ...
    private void refreshCandidate(UUID candidateId) {
        ...
        if (readyForReview && !repository.jobExists(candidateId)) {
            repository.saveJob(ShadowGenerationJob.builder()
                    .candidate(candidate).status(JobStatus.QUEUED).attemptCount(0)
                    .queuedAt(Instant.now()).updatedAt(Instant.now())
                    .build());
        }
    }
